...
import { filteredModels } from '@/lib/ModelSearch'
...
export default function ModelSelectionPage() {
    ...
    const filteredModels = availableModels // multiword search it was last working very good
        .map((model) => {
            ...
            // Helper function to check if any token starts with the field
            const startsWithToken = (field: string) => {
                return normalizedQueryTokens.some(token => field.startsWith(token));
            };

            // Helper function to check if any token is included in the field
            const includesToken = (field: string) => {
                return normalizedQueryTokens.some(token => field.includes(token));
            };

            // Priority for model_name and organization (highest priority for startsWith, then includes)
    ...
    // Remove duplicate models based on model_string_for_api
    const uniqueModels = new Map();
    filteredModels.forEach(model => uniqueModels.set(model.model_string_for_api, model));
    const filteredUniqueModels = Array.from(uniqueModels.values());
    // const filteredUniqueModels = filteredModels
